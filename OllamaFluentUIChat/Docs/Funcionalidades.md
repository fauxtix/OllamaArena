...
- **Fluxo:**
  ...
  3. Direita: card por resposta com badges `Tokens/s`, `Eval`, `Load`, `Tokens`; ratings Gemini/ChatGPT; botões **Avaliação** (`BenchmarkEvaluationDialog` — formulário 11 métricas 1–5 + tradução de feedback, ver 4.1) e **Copiar para avaliação** (gera o prompt do juiz com `EvaluatePromptTemplate` + metadados do modelo via `/api/show`).
