...
- **Processo de Avaliação de Juízes:** Para cada resposta obtida, selecione a opção **"Copia para avaliação"** — o prompt de auditoria fica no clipboard. Abra manualmente as sessões do ChatGPT e do Gemini no seu browser e faça <kbd>Ctrl</kbd>+<kbd>V</kbd> em cada uma<kbd>Ctrl</kbd>+<kbd>V</kbd>. De seguida, selecione a opção **"Avaliação"** na aplicação. Copie a resposta do primeiro juiz no browser e cole-a no ecrã de avaliação da app. Repita o processo para o segundo juiz para guardar os veredictos na BD.
